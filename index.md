...
- [[processes/distributed-workflow/active/aura-session-runner]] — **Aura Session Runner** (created 2026-08-12; **Phase S1 ⏭ ACTIVE — the current Neurospect lane**). A screen Paul keeps open **beside Tradezella** and follows every session: the Aura checklist per day/entry, the exact steps to set a session up, what to track/tag, and what to mark on each chart. Opened because of the line that matters most in the estate — *"right now I have only been building and testing the app with you, not actually using it"*: the platform has **never been used for real practice** and every number in E1–E6 came from fixtures. **It is a projection, not content authoring** — `concepts/mastery/aura/checklist.md` is *already* an 8-phase session checklist (pre-market readiness → HTF framing → Sequential SMT → entry → in-trade → exit → circuit-breakers → post-market review) with a **"Per-Trade Card (copy one per setup)"**, and `rules.md` already holds R18–R54 grouped A–E; the app becomes a read-only view over them, matching the existing wiki-projected rubric pattern. **⭐ It unlocks B2 rather than bypassing it:** B1's `DON'T BUILD` verdict rested on three free Tradezella instruments reading genuine ZERO, but that was measured against **2 sessions / 1 trade / 8 minutes** and the same section records **"with zero rules defined"** — B2 asks for twenty sessions against rules never written into Tradezella, so the gate was blocked on a **missing instrument**, not on Paul's discipline. S1 builds that instrument and **writes nothing** (capture goes into Tradezella's own fields, which *is* B2), so it cannot prejudge the outcome. **S2 absorbs B4** (E5 pre-commitment, consistency/`CI`, `SELF-REPORTED` quarantine, and the **two-clock** finding — a pre-commitment is a claim about ordering in time, so S2 cannot dodge choosing a clock). Carries an unmeasured **boundary probe** that gates the markup piece — what Tradezella's backtest chart permits — with an explicit warning that **Pine must not be assumed to work** because the older `neurospect-app` shipped a Pine script; Tradezella ≠ TradingView. Origin: [[processes/distributed-workflow/active/backtest-companion]].

- [[processes/distributed-workflow/active/backtest-companion]] — **Backtest Companion** (created 2026-08-10; **B1 ✅ complete 2026-08-11 — verdict DON'T BUILD the Tradezella importer; B3 ✅ closed 2026-08-12 *locally, not hosted*; Phase B4 capture-first ⏭ ACTIVE; B2 — Paul's twenty Tradezella sessions — still gates everything and is still unrun**). **The 2026-08-12 pivot:** Paul decided hosting is premature, so B3 shipped a one-command **local Docker daily driver** (`docker compose up -d`, :8001 / :5174, sharing the dev DB on :5433) instead of the Render deployment, render-verified with 12 artifacts in `neurospect-learn/api/docs/evidence/b3-local/`; the Render + R2 path stays written and proven but **PARKED and unprovisioned**, and its B3b boot prompt is retired **unrun** because executing it would spend money against a reversed decision. That session also found an `app` healthcheck that had failed **1350 times in a row while the site served perfectly** (nginx binds IPv4 only, no IPv6 stack, musl resolves `localhost` to `::1` first) — and where the obvious fix, `listen [::]:80;`, would have made nginx refuse to **start**. It proved the local persistence claim end to end (upload a capture → force-recreate both containers → thumbnail still renders with the rep intact), which is the local answer to the question R2 answers on Render. Opened the same day the enforcement workstream closed, on Paul's redirect: **Tradezella is now the backtesting tool, chosen and kept**, and the question is what `neurospect-learn` becomes *around* a session it does not host — consistency and discipline during the session, capture and storage of what came out of it, insight across sessions — and whether that generalises into a **companion** positioning that integrates with existing trading apps for product-market fit. Holds **17 open questions** grouped so the **boundary facts come first** (what Tradezella actually exposes — API? export? what does its ToS permit? — because that answer changes the shape of everything downstream, the E4 "measure the thing that can kill the design first" lesson). Flags two hazards up front: **`neurospect-learn` is deployed nowhere** and an integration with a hosted third party may make hosting a genuine prerequisite for the first time (overturning design decision #4); and **n=1** — Paul is the only user this platform has ever had and every number in E1–E6 came from fixtures, so no PMF claim may rest on it. Also carries a **§Recommended tooling** section answering Paul's "council of 5" question: the four installed council skills (`conclave`/`inquest`/`assay`/`vigil`) are all **convergent** — they interrogate an artifact that already exists — and **none fits discovery**, so the recommendation was to author one *divergent* sibling with five orthogonal lenses over the evidence chain of a product bet, boundary-first and able to halt the rest. **That skill was authored 2026-08-11: `⛏ prospect`** (`C:\Users\PaulRussell\.claude\skills\prospect\`, lenses `claimant`/`scout`/`scholar`/`canvasser`/`devil`; signature gate = a boundary probe that **halts the council on CLOSED**; source tier `OBSERVED→MARKETED` where a vendor claim is never a design premise; four kinds of absence including **TRIED-AND-FAILED**; and boundary facts that **expire** with a re-check trigger). It was dry-run against the 17 questions before being called done, and the tracker's new **§Dry-run** section records what that exposed — the mapping is **not** 1:1 (Q4/Q10b/Q11 are internal design questions no external lens should get; Q3 and Q17 need splitting; the skill asks three questions the 17 do not). **B1 must read §Dry-run before briefing a council.** Origin record: [[concepts/roadmap/ideas/backtest-companion-layer]].
